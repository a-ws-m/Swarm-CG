...
from swarmcg.shared import catch_warnings, exceptions, math_utils, styling
...
def load_aa_data(ns):
    ...
    ns.all_atoms = (
        dict()
    )  # atom centered connectivity + atom type + heavy atom boolean + bead(s) to which the atom belongs (can belong to multiple beads depending on mapping)
    ns.all_aa_mols = (
        []
    )  # atom groups for each molecule of interest, in case we use several and average the distributions across many molecules, as we would do for membranes analysis

    if ns.molname_in == None:

        molname_atom_group = ns.aa_universe.atoms[
            0
        ].fragment  # select the AA connected graph for the first moltype found in TPR
        ...
        for i in range(len(molname_atom_group)):
            ...
            atom_type = ns.aa_universe.atoms[i].type[
                0
            ]  # this was to check for hydrogens but we don"t need it atm
            atom_charge = ns.aa_universe.atoms[i].charge
            atom_heavy = True
            if atom_type[0].upper() == "H":
                atom_heavy = False

            ns.all_atoms[atom_id] = {
                "conn": set(),
                "atom_type": atom_type,
...
def read_ndx_atoms2beads(ns):
    ...
    with open(ns.cg_map_filename, "r") as fp:
        ...
        ndx_lines = [
            ndx_line.strip().split(";")[0] for ndx_line in ndx_lines
        ]  # split for comments
        ...
        for i in range(len(ndx_lines)):
            ...
            if ndx_line != "":
                ...
                if bool(re.search("\[.*\]", ndx_line)):
                    ...
                    try:
                        bead_atoms_id = [
                            int(atom_id) - 1 for atom_id in ndx_line.split()
                        ]  # retrieve indexing from 0 for atoms IDs for MDAnalysis
                        ns.all_beads[current_bead_id]["atoms_id"].extend(
                            bead_atoms_id
                        )  # all atoms included in current bead

                        for (
                            atom_id
                        ) in (
                            bead_atoms_id
                        ):  # bead to which each atom belongs (one atom can belong to multiple beads if there is split-mapping)
...
def get_atoms_weights_in_beads(ns):
    ...
    for bead_id in ns.all_beads:
        ...
        for atom_id in beads_atoms_counts:
            ns.atom_w[bead_id][atom_id] = round(
                beads_atoms_counts[atom_id] / ns.atoms_occ_total[atom_id], 3
            )
    ...
    for bead_id in ns.atom_w:
        try:
            ...
            if ns.mapping_type == "COM":
                ns.mda_beads_atom_grps[bead_id] = mda.AtomGroup(
                    [atom_id for atom_id in ns.atom_w[bead_id]], ns.aa_universe
                )
                ns.mda_weights_atom_grps[bead_id] = np.array(
                    [
                        ns.atom_w[bead_id][atom_id] * ns.aa_universe.atoms[atom_id].mass
                        for atom_id in ns.atom_w[bead_id]
                    ]
                )
...
def update_cg_itp_obj(ns, parameters_set, update_type):
    ...
    for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
        if ns.exec_mode == 1:
            itp_obj["constraint"][i]["value"] = round(
                parameters_set[i], 3
            )  # constraint - distance

    for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
        if ns.exec_mode == 1:
            itp_obj["bond"][i]["value"] = round(
                parameters_set[ns.opti_cycle["nb_geoms"]["constraint"] + i], 3
            )  # bond - distance
            itp_obj["bond"][i]["fct"] = round(
                parameters_set[
                    ns.opti_cycle["nb_geoms"]["constraint"]
    ...
    for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
        if ns.exec_mode == 1:
            ...
            itp_obj["angle"][i]["fct"] = round(
                parameters_set[ns.opti_cycle["nb_geoms"]["bond"] + i], 2
            )  # angle - force constant
        ...
        if ns.exec_mode == 1:
            itp_obj["dihedral"][i]["value"] = round(
                parameters_set[
                    ns.opti_cycle["nb_geoms"]["constraint"]
                    + 2 * ns.opti_cycle["nb_geoms"]["bond"]
                    + 2 * ns.opti_cycle["nb_geoms"]["angle"]
                    + i
                ],
                2,
            )  # dihedral - value
            itp_obj["dihedral"][i]["fct"] = round(
                parameters_set[
                    ...
                    + 2 * ns.opti_cycle["nb_geoms"]["bond"]
                    + 2 * ns.opti_cycle["nb_geoms"]["angle"]
                    + ns.opti_cycle["nb_geoms"]["dihedral"]
                    + i
                ],
                2,
...
def get_search_space_boundaries(ns):
    ...
    if ns.opti_cycle["nb_geoms"]["bond"] > 0:
        ...
        search_space_boundaries.extend(
            [[0, ns.default_max_fct_bonds_opti]] * ns.opti_cycle["nb_geoms"]["bond"]
        )  # bonds force constants

    if ns.opti_cycle["nb_geoms"]["angle"] > 0:
        if ns.exec_mode == 1:
            search_space_boundaries.extend(
                ns.domains_val["angle"]
            )  # angles equilibrium values

        for grp_angle in range(
            ns.opti_cycle["nb_geoms"]["angle"]
    ...
    if ns.opti_cycle["nb_geoms"]["dihedral"] > 0:
        if ns.exec_mode == 1:
            search_space_boundaries.extend(
                ns.domains_val["dihedral"]
            )  # dihedrals equilibrium values

        for grp_dihedral in range(
            ns.opti_cycle["nb_geoms"]["dihedral"]
            ...
            if ns.cg_itp["dihedral"][grp_dihedral]["func"] == 2:
                ...
                ns.cg_itp["dihedral"][grp_dihedral]["func"]
                in config.dihedral_func_with_mult
            ):
                search_space_boundaries.extend(
                    [
                        [
...
def get_initial_guess_list(ns, nb_particles):
    ...
    initial_guess_list = (
        []
    )  # array of arrays (inner arrays are the values used for particles initialization)
    ...
    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
            input_guess.append(
                min(
                    max(
                        ns.out_itp["constraint"][i]["value"],
                        ns.domains_val["constraint"][i][0],
                    ),
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
            input_guess.append(
                min(
                    max(ns.out_itp["bond"][i]["value"], ns.domains_val["bond"][i][0]),
    ...
    for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
        input_guess.append(
            min(max(ns.out_itp["bond"][i]["fct"], 0), ns.default_max_fct_bonds_opti)
        )  # bonds force constants
    ...
    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
            input_guess.append(
                min(
                    max(ns.out_itp["angle"][i]["value"], ns.domains_val["angle"][i][0]),
    ...
    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            input_guess.append(
                min(
                    max(
                        ns.out_itp["dihedral"][i]["value"],
                        ns.domains_val["dihedral"][i][0],
                    ),
    ...
    if ns.opti_cycle["nb_cycle"] > 1:
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                if (
                    ns.all_best_emd_dist_geoms["constraints"][i]
                    != config.sim_crash_EMD_indep_score
                ):
                    input_guess.append(
                        ns.all_best_params_dist_geoms["constraints"][i]["params"][0]
                    )
                else:
                    input_guess.append(
                        min(
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
                if (
                    ns.all_best_emd_dist_geoms["bonds"][i]
                    != config.sim_crash_EMD_indep_score
                ):
                    input_guess.append(
                        ns.all_best_params_dist_geoms["bonds"][i]["params"][0]
                    )
                else:
                    input_guess.append(
                        min(
                            max(
                                ns.out_itp["bond"][i]["value"],
                                ns.domains_val["bond"][i][0],
                            ),
                            ns.domains_val["bond"][i][1],
                        )
                    )
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
            if (
                ...
                input_guess.append(
                    ns.all_best_params_dist_geoms["bonds"][i]["params"][1]
                )
            else:
                input_guess.append(
                    min(
                        max(ns.out_itp["bond"][i]["fct"], 0),
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
                if (
                    ns.all_best_emd_dist_geoms["angles"][i]
                    != config.sim_crash_EMD_indep_score
                ):
                    input_guess.append(
                        ns.all_best_params_dist_geoms["angles"][i]["params"][0]
                    )
                else:
                    input_guess.append(
                        min(
                            max(
                                ns.out_itp["angle"][i]["value"],
                                ns.domains_val["angle"][i][0],
                            ),
                            ns.domains_val["angle"][i][1],
                        )
                    )
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
            if (
                ...
                input_guess.append(
                    ns.all_best_params_dist_geoms["angles"][i]["params"][1]
                )
            else:
                if ns.cg_itp["angle"][i]["func"] == 1:
                    input_guess.append(
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
                if (
                    ns.all_best_emd_dist_geoms["dihedrals"][i]
                    != config.sim_crash_EMD_indep_score
                ):
                    input_guess.append(
                        ns.all_best_params_dist_geoms["dihedrals"][i]["params"][0]
                    )
                else:
                    input_guess.append(
                        min(
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            if (
                ...
                input_guess.append(
                    ns.all_best_params_dist_geoms["dihedrals"][i]["params"][1]
                )
            else:
                if ns.cg_itp["dihedral"][i]["func"] == 2:
                    input_guess.append(
    ...
    # optionally second particle is initialized as input parameters ONLY at start of opti cycle 1
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                input_guess.append(
                    min(
                        max(
                            ns.out_itp["constraint"][i]["value_user"],
                            ns.domains_val["constraint"][i][0],
                        ),
                        ns.domains_val["constraint"][i][1],
                    )
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
            input_guess.append(
                min(
                    max(ns.out_itp["bond"][i]["fct_user"], 0),
                    ns.default_max_fct_bonds_opti,
                )
            )
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
            if ns.cg_itp["angle"][i]["func"] == 1:
                input_guess.append(
                    min(
                        max(ns.out_itp["angle"][i]["fct_user"], 0),
                        ns.default_max_fct_angles_opti_f1,
                    )
                )
            elif ns.cg_itp["angle"][i]["func"] == 2:
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
                input_guess.append(
                    min(
                        max(
                            ns.out_itp["dihedral"][i]["value_user"],
                            ns.domains_val["dihedral"][i][0],
                        ),
                        ns.domains_val["dihedral"][i][1],
                    )
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            if ns.cg_itp["dihedral"][i]["func"] == 2:
                input_guess.append(
                    min(
                        max(
                            ns.out_itp["dihedral"][i]["fct_user"],
                            -ns.default_abs_range_fct_dihedrals_opti_func_without_mult,
                        ),
                        ns.default_abs_range_fct_dihedrals_opti_func_without_mult,
                    )
    ...
    for i in range(num_particle_random_start, nb_particles):
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(
                    ns.out_itp["constraint"][j]["value"]
                    - config.bond_dist_guess_variation
                    * ns.val_guess_fact
                    * emd_err_fact,
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["bond"]):
            ...
            draw_low = max(
                min(
                    ns.out_itp["bond"][j]["fct"]
                    * (1 - ns.fct_guess_fact * emd_err_fact),
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["angle"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(
                    ns.out_itp["angle"][j]["value"]
                    - config.angle_value_guess_variation
                    * ns.val_guess_fact
                    * emd_err_fact,
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["angle"]):
            ...
            draw_low = max(
                min(
                    ns.out_itp["angle"][j]["fct"]
                    * (1 - ns.fct_guess_fact * emd_err_fact),
                    ns.out_itp["angle"][j]["fct"]
                    - config.fct_guess_min_flat_diff_angles,
                ),
                0,
            )
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(
                    ns.out_itp["dihedral"][j]["value"]
                    - config.dihedral_value_guess_variation
                    * ns.val_guess_fact
                    * emd_err_fact,
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            ...
            if ns.out_itp["dihedral"][j]["fct"] > 0:  # if positive
                ...
                draw_low = ns.out_itp["dihedral"][j]["fct"] * (
                    1 - ns.fct_guess_fact * emd_err_fact
                )
                draw_high = ns.out_itp["dihedral"][j]["fct"] * (
                    1 + ns.fct_guess_fact * emd_err_fact
                )
            else:
                # initial variations range
            ...
            if ns.cg_itp["dihedral"][j]["func"] == 2:
                draw_low = max(
                    min(
                        draw_low,
                        ns.out_itp["dihedral"][j]["fct"]
                        - config.fct_guess_min_flat_diff_dihedrals_without_mult,
                    ),
                    ns.default_abs_range_fct_dihedrals_opti_func_without_mult,
                )
                draw_high = min(
                ...
                draw_low = max(
                    min(
                        draw_low,
                        ns.out_itp["dihedral"][j]["fct"]
                        - config.fct_guess_min_flat_diff_dihedrals_with_mult,
                    ),
                    -ns.default_abs_range_fct_dihedrals_opti_func_with_mult,
                )
                draw_high = min(
        ...
        initial_guess_list.append(
            init_guess
        )  # register new particle, built during this loop
...
def map_aa2cg_traj(ns):
    ...
    for bead_id in range(len(ns.cg_itp["atoms"])):
        if not ns.cg_itp["atoms"][bead_id]["bead_type"].startswith(
            "v"
        ):  # bead is NOT a virtual site
    ...
    for bead_id in range(len(ns.cg_itp["atoms"])):
        if ns.cg_itp["atoms"][bead_id]["bead_type"].startswith("v"):
            ...
            if ns.cg_itp["atoms"][bead_id]["vs_type"] == 2:
                vs_def_beads_ids = ns.cg_itp["virtual_sites2"][bead_id][
                    "vs_def_beads_ids"
                ]
            ...
            if ns.cg_itp["atoms"][bead_id]["vs_type"] == 3:
                vs_def_beads_ids = ns.cg_itp["virtual_sites3"][bead_id][
                    "vs_def_beads_ids"
                ]
            ...
            if ns.cg_itp["atoms"][bead_id]["vs_type"] == 4:
                vs_def_beads_ids = ns.cg_itp["virtual_sites4"][bead_id][
                    "vs_def_beads_ids"
                ]
            ...
            if ns.cg_itp["atoms"][bead_id]["vs_type"] == "n":
                vs_def_beads_ids = ns.cg_itp["virtual_sitesn"][bead_id][
                    "vs_def_beads_ids"
                ]
...
@catch_warnings(
    RuntimeWarning
)  # ignore the warning "divide by 0 encountered in true_divide" while calculating sigma
def perform_BI(ns):
    ...
    if not ns.performed_init_BI["bond"] and ns.opti_cycle["nb_geoms"]["bond"] > 0:
        ...
        for grp_bond in range(ns.opti_cycle["nb_geoms"]["bond"]):
            ...
            hists_geoms_bi, std_grp_bond, avg_grp_bond, bi_xrange = ns.data_BI["bond"][
                grp_bond
            ]
            hist_geoms_modif = hists_geoms_bi**2 * (
                max(hists_geoms_bi) / max(hists_geoms_bi**2)
            )

            nb_passes = 3
            alpha = 0.55
            for _ in range(nb_passes):
                hist_geoms_modif = math_utils.ewma(
                    hist_geoms_modif, alpha, int(config.bi_nb_bins / 10)
                )

            y = -config.kB * ns.temp * np.log(hist_geoms_modif + 1)
            x = np.linspace(
                bi_xrange[0], bi_xrange[1], config.bi_nb_bins, endpoint=True
            )
            ...
            if (
                not 0
                <= ns.out_itp["bond"][grp_bond]["fct"]
                <= min(config.default_max_fct_bonds_bi, ns.default_max_fct_bonds_opti)
            ):
                ns.out_itp["bond"][grp_bond]["fct"] = (
                    min(config.default_max_fct_bonds_bi, ns.default_max_fct_bonds_opti)
                    / 2
    ...
    if not ns.performed_init_BI["angle"] and ns.opti_cycle["nb_geoms"]["angle"] > 0:
        ...
        for grp_angle in range(ns.opti_cycle["nb_geoms"]["angle"]):
            ...
            hists_geoms_bi, std_rad_grp_angle, bi_xrange = ns.data_BI["angle"][
                grp_angle
            ]
            ...
            sigma = np.where(
                y < max(y), 0.1, np.inf
            )  # this is definitely better when angles have bimodal distributions
            ...
            if func == 1:
                ...
                popt[0] = abs(
                    popt[0]
                )  # just to be safe, in case the fit yielded negative fct values but this is very unlikely since we provide good starting parameters for the fit
                ...
                try:
                    ...
                    if (
                        popt[0] < 0
                    ):  # correct the negative force constant that can result from the fit of stiff angles at values close to 180
                        popt[0] = (
                            config.default_max_fct_angles_bi * 0.8
                        )  # stiff is most probably max fct value, so get close to it
            ...
            if func == 1:
                if (
                    not 0
                    <= ns.out_itp["angle"][grp_angle]["fct"]
                    <= min(
                        config.default_max_fct_angles_bi,
                        ns.default_max_fct_angles_opti_f1,
                    )
                ):
                    ns.out_itp["angle"][grp_angle]["fct"] = (
    ...
    if (
        not ns.performed_init_BI["dihedral"]
        and ns.opti_cycle["nb_geoms"]["dihedral"] > 0
    ):
        ...
        for grp_dihedral in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            ...
            hists_geoms_bi, std_rad_grp_dihedral, avg_rad_grp_dihedral, bi_xrange = (
                ns.data_BI["dihedral"][grp_dihedral]
            )
            y = -config.kB * ns.temp * np.log(hists_geoms_bi + 1)
            x = np.linspace(
                np.deg2rad(bi_xrange[0]),
            ...
            if (
                ns.exec_mode == 2
            ):  # in Mode 2, make the fit according to the equilibrium value provided by user
                avg_rad_grp_dihedral = np.deg2rad(
                    ns.cg_itp["dihedral"][grp_dihedral]["value_user"]
                )
            ...
            if func in config.dihedral_func_with_mult:
                multiplicity = ns.cg_itp["dihedral"][grp_dihedral][
                    "mult"
                ]  # multiplicity stays the same as in input CG ITP, it"s only during model_prep that we could compare between different multiplicities
            ...
            if (
                ns.exec_mode == 1
            ):  # in Mode 1, use the fitted value as equilibrium value (but stay within range)
                ...
                ns.out_itp["dihedral"][grp_dihedral]["value"] = np.rad2deg(
                    popt[1]
                )  # we will apply limits of equilibrium values later

            print(
                "  Dihedral group",
                grp_dihedral + 1,
                "estimated force constant BEFORE MODIFIER:",
                round(popt[0], 2),
            )
            ...
            if func in config.dihedral_func_with_mult:
                if (
                    ...
                    <= ns.out_itp["dihedral"][grp_dihedral]["fct"]
                    <= min(
                        -config.default_abs_range_fct_dihedrals_bi_func_with_mult,
                        -ns.default_abs_range_fct_dihedrals_opti_func_with_mult,
                    )
                    ...
                    ns.out_itp["dihedral"][grp_dihedral]["fct"] = (
                        np.sign(ns.out_itp["dihedral"][grp_dihedral]["fct"])
                        * min(
                            config.default_abs_range_fct_dihedrals_bi_func_with_mult,
                            ns.default_abs_range_fct_dihedrals_opti_func_with_mult,
                        )
...
def compare_models(
    ...
    calc_sasa=False,
    record_best_indep_params=False,
):
    ...
    (
        ...
        row_wise_ranges["max_range_bonds"],
        row_wise_ranges["max_range_angles"],
    ...
    if not ns.atom_only:
            ...
            for vs_type in ["2", "3", "4", "n"]:
                try:
                    for bead_id in ns.cg_itp["virtual_sites" + vs_type]:
                        for vs_def_bead_id in ns.cg_itp["virtual_sites" + vs_type][
                            bead_id
                        ]["vs_def_beads_ids"]:
        ...
        ag_mol = mda.AtomGroup(
            [bead_id for bead_id in range(len(ns.cg_itp["atoms"]))], ns.cg_universe
        )
        ...
        if calc_sasa:
            ...
            if ns.sasa_cg == None:
                return (
                    ...
                    0,
                    None,
                )  # ns.sasa_cg == None will be checked in eval_function and worst score will be attributed
    ...
    for grp_constraint in range(ns.cg_itp["nb_constraints"]):
        ...
        if manual_mode:
            ...
            constraints[grp_constraint]["AA"]["avg"] = ns.cg_itp["constraint"][
                grp_constraint
            ]["avg"]
            constraints[grp_constraint]["AA"]["hist"] = ns.cg_itp["constraint"][
                grp_constraint
            ]["hist"]

        for i in range(1, len(constraints[grp_constraint]["AA"]["hist"]) - 1):
            if (
                constraints[grp_constraint]["AA"]["hist"][i - 1] > 0
                or constraints[grp_constraint]["AA"]["hist"][i] > 0
                or constraints[grp_constraint]["AA"]["hist"][i + 1] > 0
            ):
                constraints[grp_constraint]["AA"]["x"].append(
                    np.mean(ns.bins_constraints[i : i + 2])
                )
                constraints[grp_constraint]["AA"]["y"].append(
                    constraints[grp_constraint]["AA"]["hist"][i]
                )
        ...
        if not ns.atom_only:
            try:
                ...
                for i in range(1, len(constraint_hist) - 1):
                    if (
                        constraint_hist[i - 1] > 0
                        or constraint_hist[i] > 0
                        or constraint_hist[i + 1] > 0
                    ):  # TODO: find real min/max correctly, currently this code is garbage (here or nearby) and not robust to changes in bandwidth, in particular for small bandwidths
                        constraints[grp_constraint]["CG"]["x"].append(
                            np.mean(ns.bins_constraints[i : i + 2])
                        )
                        constraints[grp_constraint]["CG"]["y"].append(
                            constraint_hist[i]
                        )

                domain_min = min(
                    constraints[grp_constraint]["AA"]["x"][0],
                    constraints[grp_constraint]["CG"]["x"][0],
                )
                domain_max = max(
                    constraints[grp_constraint]["AA"]["x"][-1],
                    constraints[grp_constraint]["CG"]["x"][-1],
                )
                avg_diff_grp_constraints.append(
                    emd(
                        constraints[grp_constraint]["AA"]["hist"],
                        constraints[grp_constraint]["CG"]["hist"],
                        ns.bins_constraints_dist_matrix,
                    )
                    * ns.bonds2angles_scoring_factor
                )
            except IndexError:
                msg = (
        ...
        if ns.row_x_scaling:
            ...
            if (
                row_wise_ranges["constraints"][grp_constraint][1]
    ...
    if ns.mismatch_order and not ns.atom_only:
        diff_ordered_grp_constraints = [
            x
            for _, x in sorted(
                zip(avg_diff_grp_constraints, diff_ordered_grp_constraints),
    ...
    for grp_bond in range(ns.cg_itp["nb_bonds"]):
        ...
        for i in range(1, len(bonds[grp_bond]["AA"]["hist"]) - 1):
            if (
                ...
                or bonds[grp_bond]["AA"]["hist"][i] > 0
                or bonds[grp_bond]["AA"]["hist"][i + 1] > 0
            ):
        ...
        if not ns.atom_only:
            try:
                bond_avg, bond_hist, _ = scores.get_CG_bonds_distrib(
                    ns, beads_ids=ns.cg_itp["bond"][grp_bond]["beads"], grp_type="bond"
                )
                ...
                for i in range(1, len(bond_hist) - 1):
                    if bond_hist[i - 1] > 0 or bond_hist[i] > 0 or bond_hist[i + 1] > 0:
                        bonds[grp_bond]["CG"]["x"].append(
                            np.mean(ns.bins_bonds[i : i + 2])
                        )
                        bonds[grp_bond]["CG"]["y"].append(bond_hist[i])

                domain_min = min(
                    bonds[grp_bond]["AA"]["x"][0], bonds[grp_bond]["CG"]["x"][0]
                )
                domain_max = max(
                    bonds[grp_bond]["AA"]["x"][-1], bonds[grp_bond]["CG"]["x"][-1]
                )
                avg_diff_grp_bonds.append(
                    emd(
                        bonds[grp_bond]["AA"]["hist"],
                        bonds[grp_bond]["CG"]["hist"],
                        ns.bins_bonds_dist_matrix,
                    )
                    * ns.bonds2angles_scoring_factor
                )
            except IndexError:
        ...
        if ns.row_x_scaling:
            if ns.atom_only:
                row_wise_ranges["bonds"][grp_bond] = [
                    bonds[grp_bond]["AA"]["x"][0],
                    bonds[grp_bond]["AA"]["x"][-1],
                ]
            else:
                row_wise_ranges["bonds"][grp_bond] = [domain_min, domain_max]
            if (
                row_wise_ranges["bonds"][grp_bond][1]
    ...
    if ns.mismatch_order and not ns.atom_only:
        diff_ordered_grp_bonds = [
            x
            for _, x in sorted(
                zip(avg_diff_grp_bonds, diff_ordered_grp_bonds),
                key=lambda pair: pair[0],
                reverse=True,
            )
        ]
    ...
    for grp_angle in range(ns.cg_itp["nb_angles"]):
        ...
        if manual_mode:
            angle_avg, angle_hist, _, _ = scores.get_AA_angles_distrib(
                ns, beads_ids=ns.cg_itp["angle"][grp_angle]["beads"]
            )
        ...
        for i in range(1, len(angles[grp_angle]["AA"]["hist"]) - 1):
            if (
                ...
                or angles[grp_angle]["AA"]["hist"][i] > 0
                or angles[grp_angle]["AA"]["hist"][i + 1] > 0
            ):
        ...
        if not ns.atom_only:
            ...
            for i in range(1, len(angle_hist) - 1):
                if angle_hist[i - 1] > 0 or angle_hist[i] > 0 or angle_hist[i + 1] > 0:
                    angles[grp_angle]["CG"]["x"].append(
                        np.mean(ns.bins_angles[i : i + 2])
                    )
                    angles[grp_angle]["CG"]["y"].append(angle_hist[i])

            domain_min = min(
                angles[grp_angle]["AA"]["x"][0], angles[grp_angle]["CG"]["x"][0]
            )
            domain_max = max(
                angles[grp_angle]["AA"]["x"][-1], angles[grp_angle]["CG"]["x"][-1]
            )
        ...
        if ns.row_x_scaling:
            if ns.atom_only:
                row_wise_ranges["angles"][grp_angle] = [
                    angles[grp_angle]["AA"]["x"][0],
                    angles[grp_angle]["AA"]["x"][-1],
                ]
            else:
                row_wise_ranges["angles"][grp_angle] = [domain_min, domain_max]
            if (
                row_wise_ranges["angles"][grp_angle][1]
    ...
    if ns.mismatch_order and not ns.atom_only:
        diff_ordered_grp_angles = [
            x
            for _, x in sorted(
                zip(avg_diff_grp_angles, diff_ordered_grp_angles),
                key=lambda pair: pair[0],
                reverse=True,
            )
        ]
    ...
    for grp_dihedral in range(ns.cg_itp["nb_dihedrals"]):
        ...
        if manual_mode:
            dihedral_avg, dihedral_hist, _, _ = scores.get_AA_dihedrals_distrib(
                ns, beads_ids=ns.cg_itp["dihedral"][grp_dihedral]["beads"]
            )
            dihedrals[grp_dihedral]["AA"]["avg"] = dihedral_avg
            dihedrals[grp_dihedral]["AA"]["hist"] = dihedral_hist
        else:  # use atomistic reference that was loaded by the optimization routines
            dihedrals[grp_dihedral]["AA"]["avg"] = ns.cg_itp["dihedral"][grp_dihedral][
                "avg"
            ]
            dihedrals[grp_dihedral]["AA"]["hist"] = ns.cg_itp["dihedral"][grp_dihedral][
                "hist"
            ]
        ...
        for i in range(1, len(dihedrals[grp_dihedral]["AA"]["hist"]) - 1):
            if (
                ...
                or dihedrals[grp_dihedral]["AA"]["hist"][i] > 0
                or dihedrals[grp_dihedral]["AA"]["hist"][i + 1] > 0
            ):
                dihedrals[grp_dihedral]["AA"]["x"].append(
                    np.mean(ns.bins_dihedrals[i : i + 2])
                )
                dihedrals[grp_dihedral]["AA"]["y"].append(
                    dihedrals[grp_dihedral]["AA"]["hist"][i]
                )
        ...
        if not ns.atom_only:
            ...
            for i in range(1, len(dihedral_hist) - 1):
                if (
                    dihedral_hist[i - 1] > 0
                    or dihedral_hist[i] > 0
                    or dihedral_hist[i + 1] > 0
                ):
                    dihedrals[grp_dihedral]["CG"]["x"].append(
                        np.mean(ns.bins_dihedrals[i : i + 2])
                    )
            ...
            domain_min = min(
                ...
                dihedrals[grp_dihedral]["CG"]["x"][0],
            )
            domain_max = max(
                dihedrals[grp_dihedral]["AA"]["x"][-1],
                dihedrals[grp_dihedral]["CG"]["x"][-1],
            )
        ...
        if ns.row_x_scaling:
            ...
            if (
                row_wise_ranges["dihedrals"][grp_dihedral][1]
    ...
    if ns.mismatch_order and not ns.atom_only:
        diff_ordered_grp_dihedrals = [
            x
            for _, x in sorted(
                zip(avg_diff_grp_dihedrals, diff_ordered_grp_dihedrals),
                key=lambda pair: pair[0],
                reverse=True,
            )
        ]
    ...
    if larger_group > ncols:
        ...
        if not ns.mismatch_order:
            print(
                "Distributions groups will be displayed using the CG ITP file groups ordering"
            )
        else:
            print(
    ...
    nrows -= sum(
        [
            ns.cg_itp["nb_constraints"] == 0,
            ns.cg_itp["nb_bonds"] == 0,
            ns.cg_itp["nb_angles"] == 0,
    ...
    if ns.cg_itp["nb_constraints"] != 0:
        ...
        for i in range(ncols):
            if i < ns.cg_itp["nb_constraints"]:
                ...
                ax[nrow][i].plot(
                    constraints[grp_constraint]["AA"]["avg"],
                    0,
                    color=config.atom_color,
                    marker="D",
                )
                ...
                if not ns.atom_only:
                    ...
                    ax[nrow][i].plot(
                        constraints[grp_constraint]["CG"]["avg"],
                        0,
                        color=config.cg_color,
                        marker="D",
                    )
    ...
    if ns.cg_itp["nb_bonds"] != 0:
        ...
        for i in range(ncols):
            if i < ns.cg_itp["nb_bonds"]:
                ...
                ax[nrow][i].plot(
                    bonds[grp_bond]["AA"]["avg"], 0, color=config.atom_color, marker="D"
                )

                if not ns.atom_only:
                    ax[nrow][i].set_title(
                        f"Bond grp {grp_bond + 1} - EMD Δ {round(avg_diff_grp_bonds[grp_bond], 3)}"
                    )
                    if config.use_hists:
                        ax[nrow][i].step(
                            bonds[grp_bond]["CG"]["x"],
                    ...
                    ax[nrow][i].set_title(
                        f"Bond grp {grp_bond + 1} - Avg {round(avg_diff_grp_bonds[grp_bond], 3)} nm"
                    )
                    print(
                        f"Bond {grp_bond + 1} -- AA Avg: {round(bonds[grp_bond]['AA']['avg'], 3)}"
                    )
                ax[nrow][i].grid(zorder=0.5)
                if ns.row_x_scaling:
                    ax[nrow][i].set_xlim(
                        np.mean(row_wise_ranges["bonds"][grp_bond])
                        - row_wise_ranges["max_range_bonds"] / 2 * 1.1,
                        np.mean(row_wise_ranges["bonds"][grp_bond])
                        + row_wise_ranges["max_range_bonds"] / 2 * 1.1,
                    )
                if i % 2 == 0:
        ...
        for i in range(ncols):
            if i < ns.cg_itp["nb_angles"]:
                ...
                if not ns.atom_only:
                    ...
                    print(
                        f"Angle {grp_angle + 1} -- AA Avg: {round(angles[grp_angle]['AA']['avg'], 1)}"
                    )
                ax[nrow][i].grid(zorder=0.5)
                if ns.row_x_scaling:
                    ax[nrow][i].set_xlim(
                        np.mean(row_wise_ranges["angles"][grp_angle])
                        - row_wise_ranges["max_range_angles"] / 2 * 1.1,
                        np.mean(row_wise_ranges["angles"][grp_angle])
                        + row_wise_ranges["max_range_angles"] / 2 * 1.1,
                    )
                if i % 2 == 0:
        ...
        for i in range(ncols):
            if i < ns.cg_itp["nb_dihedrals"]:
                ...
                if not ns.atom_only:
                    ...
                    print(
                        f"Dihedral {grp_dihedral + 1} -- AA Avg: {round(dihedrals[grp_dihedral]['AA']['avg'], 1)}"
                    )
                ax[nrow][i].grid(zorder=0.5)
                if ns.row_x_scaling:
                    ax[nrow][i].set_xlim(
    ...
    if not ns.atom_only:
        ...
        for i in range(ns.cg_itp["nb_constraints"]):
            ...
            if record_best_indep_params:
                if dist_pairwise < ns.all_best_emd_dist_geoms["constraints"][i]:
                    ...
                    ns.all_best_params_dist_geoms["constraints"][i]["params"] = [
                        ns.out_itp["constraint"][i]["value"]
                    ]
        ...
        for i in range(ns.cg_itp["nb_bonds"]):
            ...
            if record_best_indep_params:
                if dist_pairwise < ns.all_best_emd_dist_geoms["bonds"][i]:
                    ...
                    ns.all_best_params_dist_geoms["bonds"][i]["params"] = [
                        ns.out_itp["bond"][i]["value"],
        ...
        for i in range(ns.cg_itp["nb_angles"]):
            ...
            if record_best_indep_params:
                if dist_pairwise < ns.all_best_emd_dist_geoms["angles"][i]:
                    ...
                    ns.all_best_params_dist_geoms["angles"][i]["params"] = [
                        ns.out_itp["angle"][i]["value"],
        ...
        for i in range(ns.cg_itp["nb_dihedrals"]):
            ...
            if record_best_indep_params and not ignore_dihedrals:
                if dist_pairwise < ns.all_best_emd_dist_geoms["dihedrals"][i]:
                    ...
                    ns.all_best_params_dist_geoms["dihedrals"][i]["params"] = [
                        ns.out_itp["dihedral"][i]["value"],
                        ns.out_itp["dihedral"][i]["fct"],
                    ]
        ...
        fit_score_total = (
            fit_score_constraints_bonds + fit_score_angles + fit_score_dihedrals
        )
        ...
        (
            ...
            round(fit_score_total, 3),
            round(fit_score_constraints_bonds, 3),
            round(fit_score_angles, 3),
            round(fit_score_dihedrals, 3),
        )
        all_dist_pairwise += "\n"
        print()
        print(
            "Using bonds to angles/dihedrals (C) scoring constant:",
            ns.bonds2angles_scoring_factor,
        )
        print()
        print("Global fitness score:", fit_score_total, "(lower is better)", flush=True)
        print(
            "  Bonds/Constraints constribution to fitness score:",
            fit_score_constraints_bonds,
            flush=True,
        )
        print("  Angles constribution to fitness score:", fit_score_angles, flush=True)
        print(
            "  Dihedrals constribution to fitness score:",
            fit_score_dihedrals,
            flush=True,
        )
